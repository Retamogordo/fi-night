...
#[proc_macro]
pub fn on_state_handler(input: proc_macro::TokenStream) -> proc_macro::TokenStream  {
  quote_state_entry_handler(input)
}
...
impl ToTokens for MatchTransitionEntry {
  fn to_tokens(&self, tokens: &mut TokenStream) {
    ...
    let state_exit_handler = if self.state_exit {
      let cb_exit_ident = &state_exit_callback(state);
      quote! { (Self::#cb_exit_ident)(#data_arg, #stack_arg); }
    } else { quote! {} };

    let return_quoted 
      = if let Some(ref to) = self.to_state {
        let cb_entry_ident = &state_entry_callback(to);
        
        let state_entry_handler 
          = if self.state_entry {quote! { (Self::#cb_entry_ident)(#data_arg, #stack_arg) }} 
            else {quote! {}};

        quote! {; #state_entry_handler; #to}
    } else { 
      quote! {} 
    };
    let pop_match_guard 
    ...
    let quoted =
      if let Some(_) = self.input {
        quote! { (#state, #signal(input)) #pop_match_guard 
          => { #state_exit_handler (Self::#cb_token)(input, #data_arg, #stack_arg) #return_quoted } } 
  
      } else {     
        quote! { (#state, #signal) #pop_match_guard  
          => { #state_exit_handler (Self::#cb_token)(#data_arg, #stack_arg) #return_quoted } }
      };
...
fn state_entry_callback(state: &Ident) -> Ident {
  Ident::new(
    &format!("{}_entry", state.to_string().to_case(Case::Snake)), Span::call_site())
}
fn state_exit_callback(state: &Ident) -> Ident {
  Ident::new(
    &format!("{}_exit", state.to_string().to_case(Case::Snake)), Span::call_site())
}
...
//                           sig, sig, sig);
        ...
        let mut match_transition_tokens = match_transition_tokens.into_sorted_vec();
        ...
        if !input.is_empty() {
          ...
          bracketed!(content in input);
          while !content.is_empty() {
            let state_entry_handler: StateEntryDecl = content.parse()?;
            match_transition_tokens
              .iter_mut()
              .filter(|tr| tr.to_state.as_ref() == Some(&state_entry_handler.state))
              .for_each(|tr| {
                tr.state_entry |= state_entry_handler.is_entry;
//                tr.state_exit |= !state_entry_handler.is_entry;
              });
            match_transition_tokens
              .iter_mut()
              .filter(|tr| tr.from_state.unwrap_as_ref() == &state_entry_handler.state)
              .for_each(|tr| {
//              tr.state_entry |= state_entry_handler.is_entry;
                tr.state_exit |= !state_entry_handler.is_entry;
            });
...
struct StateEntryDecl {
//  fsm_ident: Ident,
  state: Ident,
  is_entry: bool,
}
impl Parse for StateEntryDecl {
  fn parse(input: ParseStream) -> Result<Self> {
    let state: Ident 
      = if let Ok(ty) = input.parse::<Type>() {
        match ty {
    ...
    Ok(Self { 
      ...
      is_entry: { 
        ...
        if ident != "entry" && ident != "exit" {panic!("'entry' or 'exit' expected here");}
        ident == "entry"
      }
    })
  }
}
struct StateEntrySignature {
  decl_head: StateEntryDecl,
  fsm_ident: Ident,
  body_item: Block,
}

impl Parse for StateEntrySignature {
  fn parse(input: ParseStream) -> Result<Self> {
    Ok(Self { 
      decl_head: input.parse::<StateEntryDecl>()?,
      fsm_ident: {input.parse::<Token![for]>()?; input.parse()? },
      body_item: input.parse()?,
...
fn quote_state_entry_handler(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
	let entry_handler_parsed = parse_macro_input!(input as StateEntrySignature);
  let fsm_ident = &entry_handler_parsed.fsm_ident;
  let trait_ident = &trait_ident(fsm_ident);
  let state_ident = &entry_handler_parsed.decl_head.state;
  let body_block = &entry_handler_parsed.body_item;
  let handler_ident = &if entry_handler_parsed.decl_head.is_entry { state_entry_callback(state_ident) } else { state_exit_callback(state_ident) };
